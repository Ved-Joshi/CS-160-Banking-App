...
begin
  ...
  select id
  into settle_ledger_id
  from public.ledger_accounts
  where ledger_code = 'EXT_PAYMENT_SETTLE'
  limit 1;
  ...
  if settle_ledger_id is null then
    raise exception 'Unable to locate settlement ledger account.';
  end if;

  select name
  into payee_name
  from public.payees
  where id = payment_row.payee_id;
  ...
  values
    ...
    (journal_id, settle_ledger_id, p_amount_cents, 'credit', effective_ts);
