...
set deposit_type = coalesce(deposit_type, 'check')
where deposit_type is null;

alter table public.deposits
  alter column deposit_type set default 'check';

alter table public.deposits
  alter column deposit_type set not null;
...
begin
  ...
  alter table public.deposits
    add constraint deposits_deposit_type_check
      check (deposit_type in ('cash', 'check'));
end;
$$;
